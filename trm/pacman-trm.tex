...
\begin{table}
\label{tbl:scope}
\caption{The most significant byte of a four bits of the register address, referred to as the scope, is allocated as shown.}
\begin{center}
\begin{tabular}{|lll|}
\hline
...
\begin{tabular}{|llll|}
\hline
Register    & Address   & RO/RW & Description \\
\hline
STATUS      & 0xF000 & RO & PACMAN top-level status (currently a fixed pattern) \\
ENABLES     & 0xF010 & RW & Tile power enable and individual tile enables \\
LEDS        & 0xF014 & RW & LED configuration (see text)\\
SCRA        & 0xF020 & RW & Scratch register A \\
SCRB        & 0xF024 & RW & Scratch register B \\
FW\_MAJOR   & 0xFF10 & RO & Firmware major version \\
FW\_MINOR   & 0xFF14 & RO & Firmware minor version \\
FW\_BUILD   & 0xFF18 & RO & (Reserved) \\
HW\_MAJOR   & 0xFF20 & RO & Hardware major version\\
HW\_MINOR   & 0xFF24 & RO & Hardware minor version\\
\hline
\end{tabular}
\end{center}
\end{table}
...
\begin{table}
\label{tbl:global-bitfields}
\caption{Bit fields:  discription }
\begin{center}
\begin{tabular}{|lllll|}
\hline
Register & Field             & Bits & Mask       & Description \\
\hline
ENABLES  & Tile Enables      & 0-9  & 0x000003FF & enable/disable each of the ten individual tiles \\
         & Tile Power Enable &  16  & 0x00010000 & enable/disable power to the tile power sytem \\
\hline
LEDS     & LED 3 On/Off      & 0    & 0x00000001 & turn LED 3 on or off \\
         & LED 4 On/Off      & 1    & 0x00000002 & turn LED 4 on or off \\
\hline
\end{tabular}
\end{center}
\end{table}
...
registers listed in Table~\ref{tbl:global-reg}.  The read-only
firmware registers can be used to determine the firmware version
installed, and the read-only hardware registers can be used to check
the hardware version for which the firmware was built.  The read-write
scratch registers have no impact beyond storing and retrieving the
register value, and are useful for testing the operation of the
AXI-Lite and REGBUS interfaces.
...
are available in the ENABLES register, using the bit fields described
in~\ref{tbl:global-bitfields}.  While a tile is disabled, even with
...
controlled by the LEDS register, according to the bit field
description in ~\ref{tbl:global-bitfields}.  In the current iteration,
each LED is simply turned on or off directly, but in the future, they
could be driven by specific conditions.  Two additional LEDs are
driven from the PS, and are not part of the PL firmware design.  Two
LEDs on the Trenz module are also controllable from the PS.

The global unit also provides a global status register.  In the
current implementation, this register is implemented as a fixed
pattern.
...
\draw[->, thick]
  ...
  -- node[above, font=\small] {tdata}
     node[below, font=\small] {128-bits, 21 beats}
  ($(tx.west)+(0,1.5)$);
...
%\draw[->, thick] ($(tx.east)+(0,0.0)$) -- node[above, font=\small] {b} -- ++(1,0);
%\draw[<-, thick] ($(tx.east)-(0,1.5)$) -- node[below, font=\small] {c} -- ++(1,0);
...
\begin{tabular}{|lllll|}
\hline
Register    & Address   & RWC & Per UART & Description \\
\hline
USTATUS     & 0x00  & RO  & Per UART   & UART status \\
UCONFIG     & 0x04  & RW  & Per UART   & UART configuration \\
ULOOK\_C    & 0x18  & RO  & Per UART   & Least significant four bytes of most recent TX \\
ULOOK\_D    & 0x1C  & RO  & Per UART   & Most significant four bytes of most recent TX \\
USTARTS     & 0x20  & RO  & Per UART   & count of UART starts \\
UBEATS      & 0x24  & RO  & Per UART   & count of UART beats \\
UCHAN       & 0x50  & RO  & Per UART   & UART channel ID (loopback test) \\
BSTATUS     & 0xA0  & RO  & In Common  & TX Buffer Status \\
ZERO\_CNTS  & 0xA8  & CMD & In Common  & reset counts to zero \\
\hline
\end{tabular}
\end{center}
\end{table}
...
configuration (UCONFIG) includes fields for a phase offset (in 10 ns
...
UART status registers (USTATUS, one per UART) and TX buffer status
(BSTATUS) interpretation is currently expert only, and not documented
here, but the counts (explained below).  More user friendly
quantities (derived from the status registers) are available and
described below.

The look registers (ULOOK\_C and ULOOK\_D) are the least significant
...
The number of beats encountered (UBEATS) and the number of
transmission start bits sent (USTARTS) is available for each UART.
...
\node[draw, rounded corners, minimum width=\boxwidth, minimum height=\boxshort, text width=\boxwidth, align=center] (a)     at (1,5) {RX \\ Channel \\ 0};
\node[draw, rounded corners, minimum width=\boxwidth, minimum height=\boxshort, text width=\boxwidth, align=center] (b)     at (1,1) {RX \\ Channel \\ 39};
\node[draw, rounded corners, minimum width=\boxwidth, minimum height=\boxtall, text width=\boxwidth, align=center] (rx)     at (7,3) {RX Buffer};
\node[draw, rounded corners, minimum width=\boxwidth, minimum height=\boxtall, text width=\boxwidth, align=center] (fifo) at (13,3) {FIFO\\(AXIS)};
\node at (1,3) {\large\textellipsis};  % or just {...}


\coordinate (aa) at ($(a.east)+(0,0.75)$);
\coordinate (ab) at ($(a.east)+(0,0.00)$);
\coordinate (ac) at ($(a.east)-(0,0.75)$);
\draw[<-, thick] (tx.west |- aa) -- node[above, font=\small] {udata(0)} (aa);
\draw[<-, thick] (tx.west |- ab) -- node[above, font=\small] {uvalid(0)} (ab);
\draw[->, thick] (tx.west |- ac) -- node[above, font=\small] {uready(0)} (ac);

\coordinate (ba) at ($(b.east)+(0,0.75)$);
\coordinate (bb) at ($(b.east)+(0,0.00)$);
\coordinate (bc) at ($(b.east)-(0,0.75)$);
\draw[<-, thick] (tx.west |- ba) -- node[above, font=\small] {udata(39)} (ba);
\draw[<-, thick] (tx.west |- bb) -- node[above, font=\small] {uvalid(39)} (bb);
\draw[->, thick] (tx.west |- bc) -- node[above, font=\small] {uready(39)} (bc);

% Stream Interface Arrows
\draw[->, thick]
  ($(tx.east)+(0,1.5)$)
  -- node[above, font=\small] {tdata}
     node[below, font=\small] {128-bits per beat}
  ($(fifo.west)+(0,1.5)$);
\draw[->, thick] ($(tx.east)+(0,0.0)$) -- node[above, font=\small] {tvalid} ($(fifo.west)+(0,0.0)$);
\draw[->, thick] ($(tx.east)-(0,1.0)$) -- node[above, font=\small] {tlast} ($(fifo.west)-(0,1.0)$);
\draw[<-, thick] ($(tx.east)-(0,2.0)$) -- node[above, font=\small] {tready} ($(fifo.west)-(0,2.0)$);

% POSI Output
\draw[<-, thick] (a.west) -- ++(-1.0,0) node[anchor=east, font=\small] {PISO(0)};
\draw[<-, thick] (b.west) -- ++(-1.0,0) node[anchor=east, font=\small] {PISO(39)};


\end{tikzpicture}
\caption{Data path in the RX Unit}
\label{fig:rx-unit}
\end{figure}
...
USTATUS     & 0x00  & RO  & Per UART   & UART status \\
UCONFIG     & 0x04  & RW  & Per UART   & UART configuration \\
ULOOK\_A    & 0x10  & RO  & Per UART   & \\
ULOOK\_B    & 0x14  & RO  & Per UART   & \\
ULOOK\_C    & 0x18  & RO  & Per UART   & \\
ULOOK\_D    & 0x1C  & RO  & Per UART   & \\
USTARTS     & 0x20  & RO  & Per UART   & \\
UBEATS      & 0x24  & RO  & Per UART   & \\
UUPDATES    & 0x28  & RO  & Per UART   & \\
ULOST       & 0x2C  & RO  & Per UART   & \\
UCHAN       & 0x50  & RO  & Per UART   & \\
BSTATUS     & 0xA0  & RO  & In Common  & \\
BCONFIG     & 0xA4  & RW  & In Common  & \\
ZERO\_CNTS  & 0xA8  & CMD & In Common  & \\
FCNT        & 0xB0  & RO  & In Common  & \\
FMAX        & 0xB4  & RO  & In Common  & \\
HB\_CYC     & 0xC0  & RW  & In Common  & \\
SYNC\_TS    & 0xC4  & RW  & In Common  & \\
\hline
\end{tabular}
\end{center}
\end{table}
...
BSTATUS  & (Expert)   & 31-0  & {\tt 0}x{\tt FFFFFFFF} & \\
\hline
BCONFIG  & TCYCLES    & 15-0  & {\tt 0}x{\tt 0000FFFF} & \\
...
The RX buffer configuration register (BCONFIG) includes the TCYCLES
field, which specifies the number of turn cycles included in each DMA
packet, as described above.  The sync word behavior is configured by
the number of clock cycles between heartbeats (HB\_CYC) and the
timestamp at which a rollover is recorded (SYNC\_TS).  The UART status
registers (USTATUS, one per UART) and RX buffer status (BSTATUS)
interpretation is currently expert only, and not documented here.

The look registers (ULOOK\_A through and ULOOK\_D) contain the least
...
maintained for each UART: the number of start bits received (USTARTS),
beats observed (UBEATS), and successful receptions (UUPDATES).  The
number of lost packets (ULOST), as indicated by the lost bit being
...
of words (FCNT) in the RX FIFO is available, as is the maximum number
of words (FMAX) since the last reset of zero counts command.
